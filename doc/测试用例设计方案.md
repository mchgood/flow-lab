...
| 解析器-语法/转换 | AST→FlowGraph 及解析入口 | `FlowParserServiceTest`, `MermaidParserTest`, `MermaidTransformerTest` |
...
| 引擎-核心 | 流程执行、循环检测、条件路由 | `ProcessEngineTest` |
...
### 3.2 解析器-语法/转换（`FlowParserServiceTest`/`MermaidParserTest`/`MermaidTransformerTest`）
- 节点形状解析：矩形/菱形/圆形/圆角矩形；**未实现形状（hexagon/parallelogram/trapezoid）应抛错或忽略的行为校验**。
- 边字段分离：`label` 仅做展示，`condition` 仅在使用前缀/语法时赋值，普通标签不得被求值。
- 方向与 JSON 输出（格式化/紧凑）
- 校验与错误输入（空字符串、非法语法）
- 子图测试（模块内覆盖）：
  ...
  - **新：`testParseImplicitNodesAreCapturedOnce`**（验证 transformer 统一收集节点，不重复或遗漏）
...
- `validate()` 形状校验
- 任务节点执行后上下文变量写入
- **形状冲突防护**：开始/结束执行器注册同形状时的行为（需在改造后调整期望）

### 3.7 引擎-核心（`ProcessEngineTest`）
- 简单线性流程执行完成
- 条件分支流程可执行完成
- 事件发布（流程/节点事件）
- **循环检测**：含环流程应被检测并终止/抛错，不得栈溢出
- **标签与条件分离**：文本标签不应触发 SpEL 求值，带前缀的条件才会被判定
...
- 事件监听器全流程
- 流程定义版本管理
- **环路与异常路径集成场景**：循环流程的防护、条件表达式异常的降级路径
